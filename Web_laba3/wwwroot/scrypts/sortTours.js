...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const toursList = document.querySelector(".tour-card-container");
    console.log(sortSelect1);
    console.log(sortSelect2);
    console.log(toursList);

    if (!toursList || !sortSelect1 || !sortSelect2) {
        console.error("Не удалось найти контейнер туров или поля сортировки.");
        return;
    }
    ...
    const sortTours = () => {
        ...
        if (sortOption2 !== "all") {
            sortedTours = sortedTours.filter(tour => {
                ...
                const categoryText = {
                    "rocky": "Горный тур",
                    "historical": "Историческая экскурсия",
                    "beach": "Пляжный отдых"
                };
    ...
    document.addEventListener("toursLoaded", () => {
        originalTours = Array.from(toursList.querySelectorAll(".tour-card"));
        sortTours(); // сразу применяем сортировку, если данные уже загружены
    });
});
